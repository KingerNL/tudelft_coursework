...
%
%   $Id: crg_check_data.m,v 1.6 2013/06/03 13:02:07 tsn\hofstadrhmtvd Exp $
...
if isfield(data, 'p')
    ...
    if isfield(data.head, 'pbeg')
        if (cos(data.head.pbeg)-cos(pbeg))^2 + (sin(data.head.pbeg)-sin(pbeg))^2 > crgeps^2
    ...
    if isfield(data.head, 'pend')
        if (cos(data.head.pend)-cos(pend))^2 + (sin(data.head.pend)-sin(pend))^2 > crgeps^2
    ...
    if isfield(data.head, 'pend')
        if (cos(data.head.pend)-cos(data.head.pbeg))^2 + (sin(data.head.pend)-sin(data.head.pbeg))^2 > crgeps^2
