...
variable modify  &
   ...
   string_value = "TNO_car205_60R15_parking.tir"
!
variable modify  &
...
constraint modify motion_generator  &
   motion_name = .model_1.MOT_left  &
   function = "( (IF(time-1:0,0,200/16*(time-1)/180*PI)+IF(time-3:0,0,-200/16*(time-3)/180*PI)) + (IF(time-5:0,0,-200/16*(time-5)/180*PI)+IF(time-8.1:0,0,200/16*(time-8.1)/180*PI) ) + (IF(time-9.1:0,0,200/16*(time-9.1)/180*PI)+IF(time-10.2:0,0,-200/16*(time-10.2)/180*PI) ) )"
!
constraint modify motion_generator  &
   motion_name = .model_1.MOT_right  &
   function = "( (IF(time-1:0,0,200/16*(time-1)/180*PI)+IF(time-3:0,0,-200/16*(time-3)/180*PI)) + (IF(time-5:0,0,-200/16*(time-5)/180*PI)+IF(time-8.1:0,0,200/16*(time-8.1)/180*PI) ) + (IF(time-9.1:0,0,200/16*(time-9.1)/180*PI)+IF(time-10.2:0,0,-200/16*(time-10.2)/180*PI) ) )"
...
force modify direct single_component_force  &
   single_component_force_name = .model_1.driving_torque_left  &
   action_only = off  &
   function = "( IF(time-3:-18,-18,250*(time-3)) + IF(time-4:5,5,-320*(time-4)) + IF(time-7:5,5,80*(time-7)) + IF(time-8:5,5,220*(time-8)) + IF(time-10:5,5,-225*(time-10)) )"
!
force modify direct single_component_force  &
   single_component_force_name = .model_1.driving_torque_right  &
   action_only = off  &
...
simulation set save_files = no
!
!
simulation set file_prefix="turnslip"
simulation single_run scripted  sim_script_name = .model_1.SIM_SCRIPT_1 reset_before_and_after = yes
! rename analysis
   entity modify entity = .model_1.Last_Run new = .model_1.turnslip