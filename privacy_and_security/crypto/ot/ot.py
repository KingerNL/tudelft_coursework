...
class Alice:
    ...
    def transmit(self, file_name="alice_dec.json", bob_file_names=["bob_setup_1.json", "bob_setup_2.json", "bob_setup_3.json"]):
        all_G = []
        
        for bob_file_name in bob_file_names:
            f = list(map(int, read_json(bob_file_name)))
            assert len(f) == 1, f"Bob should only request one message at a time, but got {len(f)}"

            # Compute the response for this Bob
            i = len(all_G)  # Assign message index based on order
            F = pow(compute_poly(f, i, self.G), self.privkey.d, self.pubkey.n)
            all_G.append((F * bytes_to_int(self.M[i])) % self.pubkey.n)

        write_json(file_name, all_G)
        print("All G values have been published.")
